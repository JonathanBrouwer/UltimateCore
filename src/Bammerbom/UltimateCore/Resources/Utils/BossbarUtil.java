...
public class BossbarUtil implements Listener{
	...
	public static void enable(){
		...
		enabled = true;
		Bukkit.getServer().getPluginManager().registerEvents(new BossbarUtil(), (UltimateCore) r.plugin());
		Bukkit.getServer().getScheduler().scheduleSyncRepeatingTask(r.plugin(), new Runnable() {
		    public void run() {
	...
	public static void disable(){
		...
		players.clear();
		for (Iterator<Integer> i = timers.values().iterator(); i.hasNext(); ) { int timerID = ((Integer)i.next()).intValue();
    ...
    public static void handleTeleport(final Player player, final Location loc){
        ...
        Bukkit.getScheduler().runTaskLater(r.plugin(), new Runnable(){
            public void run(){
                if (!hasBar(player)) {
                    return;
                }
    ...
    public static void setMessage(final Player player, String message, int seconds){
        ...
        timers.put(player.getUniqueId(), Integer.valueOf(Bukkit.getScheduler().runTaskTimer(r.plugin(), new Runnable(){
        public void run()
        {
